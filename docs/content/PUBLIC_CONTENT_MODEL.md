...
| --- | --- | --- | --- |
| `visibility` | `public`, `hidden`, `private` union tipi | enum veya check constraint | Yalnız `public` kayıtlar listelenir; eski `unlisted` değerleri geçiş sırasında `hidden` olarak normalize edilir; `hidden` ve `private` public render edilmez. |
| `publishState` | `draft`, `review`, `approved`, `published`, `unpublished`, `archived` union tipi | enum | Production public listelerde yalnız `published` beklenir; diğer durumlar anonymous public akışında gösterilmez. |
...
- Eski `unlisted` içerikler database geçişinde `hidden` olarak normalize edilir ve anonymous slug erişimine açılmaz.
...
## Sprint 04 publish flow ilişkisi

`docs/content/PUBLIC_PUBLISH_FLOW.md`, bu modelin Studio'dan public siteye ileride nasıl taşınacağını public taraf açısından tanımlar. Bu bölüm database migration kararı değildir; yalnız public tüketim sözleşmesini netleştirir.

### Studio alan eşleşmesi

Public taraf ileride Studio data modelinden gelen kayıtları şu public-safe alanlara indirgenmiş biçimde tüketmelidir:

| Public alan | Studio tarafındaki beklenen kaynak | Public davranış |
| --- | --- | --- |
| `publishFlowState` | Studio draft/review/publish workflow durumu | Kart ve detaylarda küçük durum etiketi olarak gösterilir. |
| `visibility` | Studio görünürlük seçimi | Yalnız `public` kayıt listelenebilir; `hidden` ve `private` kayıtlar public route içinde render edilmez. |
| `publishState` | Studio publish kararının public karşılığı | Production listelerde yalnız `published` beklenir; `draft`, `review`, `approved`, `unpublished` ve `archived` anonymous akışta render edilmez. |
| `approvalNote` | Kullanıcı onayı veya review notu | Gerçek link/görsel/kişisel bilgi bekliyorsa pasif açıklama olarak kullanılır. |
| `links.*.href` | Studio link alanı + kullanıcı onayı | Boşsa aktif link render edilmez; doluysa ayrıca onaylı olmalıdır. |
| `portrait.approvalState` | Studio Storage/görsel onay alanı | `approved` değilse final portre gibi sunulmaz. |

### Public publish kuralı
